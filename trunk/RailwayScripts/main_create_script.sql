...
CREATE TABLE CarriageFreightOrderItems 
    ( 
     carriage_id NUMBER  NOT NULL , 
     goi_id NUMBER  NOT NULL 
    ) 
;
...
CREATE TABLE CarriageType 
    ( 
     ...
     parrent_type_id NUMBER  NOT NULL 
    ) 
;
...
ALTER TABLE CarriageType 
    ADD CONSTRAINT CarriageType_PK PRIMARY KEY ( carriage_type_id ) ;
...
CREATE TABLE FeightOrderItems 
    ( 
     ...
     delivery VARCHAR2  NOT NULL 
    ) 
;



ALTER TABLE FeightOrderItems 
    ADD CONSTRAINT FeightOrderItems_PK PRIMARY KEY ( goi_id ) ;
...
BEFORE INSERT ON FeightOrderItems 
FOR EACH ROW 
WHEN (NEW.goi_id IS NULL) 
BEGIN 
    SELECT sequence_goi.NEXTVAL INTO :NEW.goi_id FROM DUAL; 
END;
/
...
CREATE TABLE Lokomotive 
    ( 
     lokomotive_id NUMBER  NOT NULL , 
     lokomotive_type VARCHAR2 (50 CHAR) CHECK ( lokomotive_type IN ('Diesel', 'Electric', 'Kleinlokomotive', 'Steam')) , 
     number_carriages NUMBER  NOT NULL , 
     railroad_type VARCHAR2  NOT NULL , 
     lokomotive_mark VARCHAR2 (50) 
    ) 
;



-- Error - Index Lokomotive_Railroad_FK has no columns

ALTER TABLE Lokomotive 
    ADD CONSTRAINT Lokomotive_PK PRIMARY KEY ( lokomotive_id ) ;



CREATE TABLE PassengerOrderItems 
    ( 
     order_id NUMBER  NOT NULL , 
...
ALTER TABLE PassengerOrderItems 
    ADD CONSTRAINT PassengerOrderItems_PK PRIMARY KEY ( poi_id ) ;
...
BEFORE INSERT ON PassengerOrderItems 
FOR EACH ROW 
WHEN (NEW.poi_id IS NULL) 
BEGIN 
...
CREATE TABLE Railroad 
    ( 
     railroad_type VARCHAR2  NOT NULL CHECK ( railroad_type IN ('electric', 'normal')) , 
...
ALTER TABLE RollingStack 
    ADD CONSTRAINT RollingStack_PK PRIMARY KEY ( rolling_stock_id ) ;
...
ALTER TABLE CarriageFreightOrderItems 
    ADD CONSTRAINT CarriageFreightOrderItems_Carriage_FK FOREIGN KEY 
    ( 
     carriage_id
    ) 
    REFERENCES Carriage 
...
--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE CarriageFreightOrderItems 
    ADD CONSTRAINT CarriageFreightOrderItems_FeightOrderItems_FK FOREIGN KEY 
    ( 
     goi_id
    ) 
    REFERENCES FeightOrderItems 
    ( 
     goi_id
    ) 
    ON DELETE CASCADE 
...
ALTER TABLE Characteristic 
    ADD CONSTRAINT Characteristic_CarriageType_FK FOREIGN KEY 
    ( 
     carriage_type_id
    ) 
    REFERENCES CarriageType 
...
--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE Characteristic 
    ADD CONSTRAINT Characteristic_Characteristic_type_FK FOREIGN KEY 
    ( 
     carriage_type_id
    ) 
